...
def main():
    ...
    tf_session = tf.Session(graph=tf_graph, config=config)
    ...
    with tf_graph.as_default():
        ...
        logger.info(tf.global_variables())
    ...
    # ## We now go through a training loop where we optimise the NN and GP. we will logger.info out the test results at
    # regular intervals.
    data_indx = 0
    results = []
